...
function wdfm_tenweb_install_notice() {
  ...
  if ( $meta_value === '' || $meta_value === FALSE ) {
    ...
    $verify_url = add_query_arg( array ('action' => 'fmc_tenweb_status'), admin_url('admin-ajax.php'));
    ?>
    <style>
      .hide {
    ...
    <div id="v2_tenweb_notice_cont" class="notice wd-notice">
      ...
      <div class="v2_content">
        <p>
          <?php echo sprintf(__('%sContact Form Maker advises:%s %sUse Image Optimizer service to optimize your images quickly and easily.%s', $prefix), '<span>','</span>', '<span>','</span>'); ?>
        </p>
      </div>
      <div class="v2_button">
        <?php WDW_FMC_Library::twbb_install_button(2); ?>
      </div>
...
if ( !is_plugin_active( '10web-manager/10web-manager.php' ) && WDFMInstance(2)->is_free ) {
...
function fm_check_tenweb_status() {
  $status_install = 0;
  $status_active = 0;
  if ( WDW_FMC_Library::is_plugin_installed('10web-manager') ) {
    $status_install = 1;
  }
  else {
  ...
  if ( WDW_FMC_Library::is_plugin_installed('10web-manager') ) {
    $old_opt_array = array();
    $new_opt_array = array( 'contact-form-maker' => 95 ); // core_id
...
add_action('wp_ajax_fmc_tenweb_status', 'fm_check_tenweb_status');