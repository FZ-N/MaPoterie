jQuery(document).on('sm_top_bar_loaded', '#sm_top_bar', function() {
	...
	if( window.smart_manager.current_selected_dashboard != 'product' ) {
		if( jQuery("#sm_top_bar_left").find('#sm_products_show_variations_span').length > 0 ) {
			jQuery("#sm_top_bar_left #sm_products_show_variations_span").remove();
		}
		return;
	}
	...
	if( jQuery("#sm_top_bar_left").find('#sm_products_show_variations_span').length == 0 ) {
		jQuery("#sm_top_bar_left").append("<label id='sm_products_show_variations_span' style='font-weight:400 !important;vertical-align: -webkit-baseline-middle;'> <input type='checkbox' name='sm_products_show_variations' id='sm_products_show_variations' value='sm_products_show_variations' style='margin-left:5px;margin-right:0px;' "+ show_variations_checked +"> Show Variations </label>");
	}

	jQuery('#sm_top_bar_left #sm_products_show_variations').off('change').on('change',function() {
...
Smart_Manager.prototype.prodAttributeInlineEdit = function(params){
	...
	jQuery('#edit_product_attributes input[name^="attribute_names"]').each( function(){
		...
		if( jQuery( "input[name='attribute_values["+index+"]']" ).attr('type') !== undefined && jQuery( "input[name='attribute_values["+index+"]']" ).attr('type') == "text" ) {
			...
			if (isTaxonomy == 1) {
				attributesEditedText += attribute_display_text [attrNm] + ': [' + editedText + ']';
