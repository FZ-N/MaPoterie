...
Smart_Manager.prototype.init = function() {
	...
	this.batch_update_action_string = {set_to:'set to', prepend:'prepend', append:'append'};
	...
	window.smart_manager.grid_height = document.documentElement.offsetHeight - 190;
...
Smart_Manager.prototype.loadTopBar = function() {
	...
	if( sm_top_bar_exists.length == 0 ) {
		...
		window.smart_manager.advancedSearchContent = "<div style='width: 100%;'> <div id='sm_advanced_search_box' style='float:left;width:74.85%' > <div id='sm_advanced_search_box_0' style='width:100%;margin-left:0.8em;margin-bottom:0.5em;'> </div>"+
		...
		if( window.smart_manager.sm_beta_pro == 0 ) { 
			...
			jQuery(document).off('click', '#sm_top_bar .sm_date_range_container').on('click', '#sm_top_bar .sm_date_range_container', function() {
		...
		let sm_top_bar = "<div id='sm_top_bar' style='font-weight:400 !important;width:"+window.smart_manager.grid_width+"px;'>"+
							"<div id='sm_top_bar_left' class='sm_beta_left'>"+
								"<label id=sm_dashboard_select_lbl style='float:left'> <select id='sm_dashboard_select' style='height:23px!important;'> </select> </label>"+
								"<div id='search_content' style='float:left; width:44%;margin-top:0.2em;margin-left:0.5em;'>"+
								...
								"</div>"+
								"<div title='Switch to "+ switchSearchType +" search' id='search_switch' class='sm_beta_left' switchSearchType='"+ switchSearchType +"'> "+ switchSearchType +" Search </div>"+
								window.smart_manager.sm_beta_smart_date_filter+
							"</div>"+
							"<div id='sm_top_bar_right' class='sm_beta_right'>"+
								...
								"<span id='save_sm_editor_grid' title='Save' class='ui-icon sm-ui-state-disabled' style='margin-top:8px;padding:8px !important;'></span>"+
								"<span id='del_sm_editor_grid' title='Delete Selected Row' class='sm-ui-state-disabled dashicons dashicons-trash sm_error_icon' style='font-size:2em;'></span>"+
								...
								"<span id='print_invoice_sm_editor_grid' class='ui-icon' style='margin-top:5px; display: none;padding:8px !important;' title='Print Invoice'></span>"+
		...
		let sm_bottom_bar = "<div id='sm_bottom_bar' style='font-weight:500 !important;color:#3892D3;width:"+window.smart_manager.grid_width+"px;'>"+
							"<div id='sm_bottom_bar_left' class='sm_beta_left'>"+
								"<div class='sm_beta_left' style='cursor: pointer;line-height:0em;' title='Load more records'><button id='sm_editor_grid_load_items' style='height:2em;border: 1px solid #3892D3;background-color: white;border-radius: 3px;cursor: pointer;line-height: 17px;color:#3892D3;'> Load More </button> </div>"+
								"<div id='sm_beta_display_records' class='sm_beta_left'></div>"+
							"</div>"+
						"</div>";

		jQuery(sm_top_bar).insertBefore("#sm_editor_grid");
		jQuery(sm_bottom_bar).insertAfter("#sm_editor_grid");
		jQuery('#sm_dashboard_select').append(window.smart_manager.dashboard_select_options);
		let dashboard_select_width = window.getComputedStyle(document.querySelector('#sm_dashboard_select'), null).width;
		let dashboard_select_lbl_width = window.getComputedStyle(document.querySelector('#sm_dashboard_select_lbl'), ':after').width;
		jQuery('#sm_dashboard_select').css('width', ( parseInt( dashboard_select_width ) + parseInt( dashboard_select_lbl_width ) ) + 'px' );
	}
...
Smart_Manager.prototype.send_request = function(params, callback) {
...
Smart_Manager.prototype.format_dashboard_column_model = function( column_model ) {
	...
	if( column_model == '' || typeof(column_model) == 'undefined' ) {
		return;
	}
	...
	let chkbox_col = {
		...
		index = 1;
	column_model.unshift(chkbox_col);
	window.smart_manager.column_names = new Array('');
	window.smart_manager.currentVisibleColumns = new Array('');

	for (i = 1; i < column_model.length; i++) {

			hidden = ( typeof(column_model[i].hidden) != 'undefined' ) ? column_model[i].hidden : true;

			column_values = (typeof(column_model[i].values) != 'undefined') ? column_model[i].values : '';

			let name = (column_model[i].hasOwnProperty('name')) ? column_model[i].name.trim() : '';
			...
			if( hidden === false ) {
				window.smart_manager.column_names[index] = column_model[i].name_display; //Array for column headers
				window.smart_manager.currentVisibleColumns[index] = column_model[i];
				index++;
			}
			...
			if (batch_enabled_flag === true) {
				window.smart_manager.column_names_batch_update[column_model[i].src] = {name: column_model[i].name_display, type:column_model[i].type, values:column_values, src:column_model[i].data};

				if( column_model[i].type == 'checkbox' ) {
					window.smart_manager.column_names_batch_update[column_model[i].src]['checkedTemplate'] = 'true';
					window.smart_manager.column_names_batch_update[column_model[i].src]['uncheckedTemplate'] = 'false';

					if( column_model[i].hasOwnProperty('checkedTemplate') ) {
						window.smart_manager.column_names_batch_update[column_model[i].src]['checkedTemplate'] = column_model[i]['checkedTemplate'];
					}

					if( column_model[i].hasOwnProperty('uncheckedTemplate') ) {
						window.smart_manager.column_names_batch_update[column_model[i].src]['uncheckedTemplate'] = column_model[i]['uncheckedTemplate'];
					}
				}
...
Smart_Manager.prototype.setDashboardModel = function (response) {
	...
	if( typeof response != 'undefined' && response != '' ) {
		window.smart_manager.sm_store_table_model = response[window.smart_manager.dashboard_key].tables;
		window.smart_manager.currentColModel = window.smart_manager.format_dashboard_column_model(response[window.smart_manager.dashboard_key].columns);
		response[window.smart_manager.dashboard_key].columns = window.smart_manager.currentColModel;
		window.smart_manager.currentDashboardModel = response[window.smart_manager.dashboard_key];
		window.smart_manager.getData();
	}
}
...
Smart_Manager.prototype.refreshBottomBar = function() {
	...
	jQuery('#sm_bottom_bar_left #sm_beta_display_records').html(msg);
}
...
Smart_Manager.prototype.enableDisableButtons = function() {
	...
	if( window.smart_manager.selectedIds.length > 0 ) {
		if( jQuery('#sm_top_bar #del_sm_editor_grid').hasClass('sm-ui-state-disabled') ) {
			jQuery('#sm_top_bar #del_sm_editor_grid').removeClass('sm-ui-state-disabled');
		}
	} else {
		if( !jQuery('#sm_top_bar #del_sm_editor_grid').hasClass('sm-ui-state-disabled') ) {
			jQuery('#sm_top_bar #del_sm_editor_grid').addClass('sm-ui-state-disabled');
		}
	}
}
...
Smart_Manager.prototype.loadGrid = function() {
	...
	window.smart_manager.hot = new Handsontable(window.smart_manager.container, {
																				  ...
																				  cells: function (row, col, prop) {
																						let cellProperties = { wordWrap: false };
			...
			if( window.smart_manager.currentVisibleColumns.length > 0 ) {
				for( let index in window.smart_manager.currentVisibleColumns ) {
					if( window.smart_manager.currentVisibleColumns[index].src == window.smart_manager.currentDashboardModel.sort_params.column ) {
						window.smart_manager.currentDashboardModel.sort_params.column = parseInt(index);
						window.smart_manager.hotPlugin.columnSortPlugin.setSortConfig([window.smart_manager.currentDashboardModel.sort_params]);
						break;
					}
				}
			}
	...
	window.smart_manager.hot.updateSettings({
		...
		afterChange: function(changes, source) {
			...
			changes.forEach(([row, prop, oldValue, newValue]) => {
				...
				if( 'row_select_checkbox' === col.key ) {
				...
				} else if( oldValue != newValue && prop != idKey ) { //for inline edit
					...
					if( jQuery('#sm_top_bar #save_sm_editor_grid').hasClass('sm-ui-state-disabled') ) {
						jQuery('#sm_top_bar #save_sm_editor_grid').removeClass('sm-ui-state-disabled');
					}
		...
		afterOnCellMouseUp: function (e, coords, td) {
			var col = this.getCellMeta(coords.row, coords.col),
			...
			if( typeof (col.type) != 'undefined' && col.type == 'sm.serialized' ) { //Code for handling serialized complex data handling
				...
				editor.set(JSON.parse(current_cell_value));
				editor.expandAll();
...
Smart_Manager.prototype.event_handler = function() {
	...
	jQuery('#collapse-menu').live('click', function() {
		...
		jQuery('#sm_top_bar').css('width',window.smart_manager.grid_width+'px');
	});
	...
	jQuery(document).off('change', '#sm_dashboard_select').on('change', '#sm_dashboard_select',function(){
		...
		if( sm_dashboard_valid == 1 ) {
			...
			jQuery('#sm_top_bar_left #search_content').html(content);
			...
			var content = 'For managing '+ sm_selected_dashboard_title +', '+ window.smart_manager.sm_success_msg + ' <a href="https://www.storeapps.org/product/smart-manager/" target="_storeapps">Pro</a>';
	...
	.off( 'click', '#search_switch').on( 'click', '#search_switch' ,function(){ //request for handling switch search types
		...
		jQuery(this).attr('title', title.replace(switchSearchType, window.smart_manager.searchType));
		jQuery(this).html(text.replace(switchSearchType, window.smart_manager.searchType));
	...
	.off( 'click', '#sm_top_bar #save_sm_editor_grid').on( 'click', '#sm_top_bar #save_sm_editor_grid' ,function(){
	...
	.off( 'click', '#sm_top_bar #del_sm_editor_grid').on( 'click', '#sm_top_bar #del_sm_editor_grid' ,function(){
		...
		if( window.smart_manager.selectedIds.length > 0 ) {
			...
			params.content = 'Are you sure you want to delete the selected ' + ( ( window.smart_manager.selectedIds.length > 1 ) ? 'records' : 'record' ) + '?';
			params.btnParams = {}
	...
	.off( 'click', "#add_sm_editor_grid").on( 'click', "#add_sm_editor_grid", function(){
		window.smart_manager.hot.alter('insert_row', 0);
	})

	// Code for handling the batch update & duplicate records functionality
	.off( 'click', "#batch_update_sm_editor_grid, .sm_beta_dropdown_content a, #export_csv_sm_editor_grid, #print_invoice_sm_editor_grid").on( 'click', "#batch_update_sm_editor_grid, .sm_beta_dropdown_content a, #export_csv_sm_editor_grid, #print_invoice_sm_editor_grid", function(){
				...
				if( id == 'export_csv_sm_editor_grid' ) { //code for handling export CSV functionality
					...
					if( window.smart_manager.selectedIds.length > 0 ) {
						...
						if( id == 'batch_update_sm_editor_grid' ) { //code for handling batch update functionality
							...
							window.smart_manager.showConfirmDialog(params);
						} else if( id == 'print_invoice_sm_editor_grid' ) { //code for handling Print Invoice functionality
...
Smart_Manager.prototype.createColumnVisibilityDialog = function() {
	...
	for( let key in window.smart_manager.currentColModel ) {
		...
		if( colObj.hasOwnProperty('allow_showhide') && colObj.allow_showhide === true ) {
			...
			colPosition = ( colObj.hasOwnProperty('position') ) ? colObj.position - 1 : '';
			...
			if( colObj.hasOwnProperty('hidden') && colObj.hidden === false ) {
				...
				if( colPosition != '' ) {
					enabledColumnsArray[colPosition] = temp;
				} else {
...
Smart_Manager.prototype.processColumnVisibility = function() {
	...
	if( enabledColumns.length > 0 ) {
		...
		for( let key in window.smart_manager.currentColModel ) {
			...
			if( colObj.hasOwnProperty('allow_showhide') && colObj.allow_showhide === true ) {
				...
				if( enabledColumnsArray.indexOf(colVal) != -1 ) {
					...
					window.smart_manager.currentVisibleColumns[index] = window.smart_manager.currentColModel[key];

					//Code for refreshing the column headers
					var name = ( colObj.hasOwnProperty('name') !== false && typeof colObj.name != 'undefined' ) ? colObj.name.trim() : '';

					if(colObj.hasOwnProperty('name_display') === false) {// added for state management
						colObj.name_display = name;
					}

					window.smart_manager.column_names[position] = colObj.name_display; //Array for column headers
					...
					if( window.smart_manager.currentColModel[key].hasOwnProperty('position') !== false ) {
						delete window.smart_manager.currentColModel[key].position;
					}
				}
			}
		}
		...
		if ( typeof (window.smart_manager.loadGrid) !== "undefined" && typeof (window.smart_manager.loadGrid) === "function" ) {
			window.smart_manager.loadGrid();
		}
...
Smart_Manager.prototype.sortColumns = function() {
	...
	if( typeof window.smart_manager.currentVisibleColumns == 'undefined' ) {
		return;
	}

	window.smart_manager.currentVisibleColumns.sort(function(a, b) {
		if( ( a.hasOwnProperty('position') != false && b.hasOwnProperty('position') != false ) ) {
			if( typeof a.position != 'undefined' && typeof b.position != 'undefined' && a.position != '' && b.position != '' ) {
...
Smart_Manager.prototype.deleteRecords = function() {
	...
	window.smart_manager.send_request(params, function(response) {
		if ( 'failed' !== response ) {
			if( jQuery('#sm_top_bar #del_sm_editor_grid').hasClass('sm-ui-state-disabled') === false ) {
				jQuery('#sm_top_bar #del_sm_editor_grid').addClass('sm-ui-state-disabled');
			}
			window.smart_manager.refresh();
...
Smart_Manager.prototype.showNotification = function( title = '', content = '' ) {
	...
	let dlg_title = ( title != '' ) ? title : 'Note',
		dlg_content = ( content != '' ) ? content : 'This feature is available only in the <a href="https://www.storeapps.org/product/smart-manager/" target="_storeapps">Pro</a> version.';
...
Smart_Manager.prototype.refreshDashboardStates = function() {
	...
	tempColModel.shift();
	// tempColModel.filter(function(val){return val});

	tempDashModel.columns = tempColModel;
	window.smart_manager.dashboardStates[window.smart_manager.dashboard_key] = JSON.stringify(tempDashModel);
}

//Function to handle the state apply at regular intervals
Smart_Manager.prototype.updateState = function() {
