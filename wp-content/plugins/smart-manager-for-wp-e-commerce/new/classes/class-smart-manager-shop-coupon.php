...
if ( ! class_exists( 'Smart_Manager_Shop_Coupon' ) ) {
	class Smart_Manager_Shop_Coupon extends Smart_Manager_Base {
		...
		public function coupons_dashboard_model ($dashboard_model) {
			...
			$post_type_col_index = sm_multidimesional_array_search('postmeta_meta_key_discount_type_meta_value_discount_type', 'data', $dashboard_model[$this->dashboard_key]['columns']);
			
			$coupon_statuses = ( function_exists('wc_get_coupon_types') ) ? wc_get_coupon_types() : array();

			$dashboard_model[$this->dashboard_key]['columns'][$post_type_col_index]['type'] = 'dropdown';
			$dashboard_model[$this->dashboard_key]['columns'][$post_type_col_index]['editor'] = 'select';
			$dashboard_model[$this->dashboard_key]['columns'][$post_type_col_index]['renderer'] = 'selectValueRenderer';
			$dashboard_model[$this->dashboard_key]['columns'][$post_type_col_index]['values'] = $coupon_statuses;
			$dashboard_model[$this->dashboard_key]['columns'][$post_type_col_index]['selectOptions'] = $coupon_statuses;
			$dashboard_model[$this->dashboard_key]['columns'][$post_type_col_index]['defaultValue'] = ( !empty( $coupon_statuses[0] ) ) ? $coupon_statuses[0] : '';
