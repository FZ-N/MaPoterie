...
if ( ! class_exists( 'Smart_Manager' ) ) {
	class Smart_Manager {
		...
		function define_constants() {
			...
			$post_types = get_post_types( array( 'show_ui' => true ), 'objects' ); //Code to get all the custom post types as dashboards
	  ...
	  //       wp_register_script ( 'sm_jquery_ui_multiselect', plugins_url ( '/assets/js/jqgrid/ui.multiselect.js', __FILE__ ), $deps, '1.10.2' );
		...
		function show_console_beta() {
			...
			<div class="wrap">
				...
				<div style="margin-bottom:1em;">
					...
					<?php
							echo 'Smart Manager ';
							echo '<sup style="vertical-align: super;color:red;">'.((SMBETAPRO === true) ? 'Pro' : 'Lite').'</sup>';
							$plug_page = '';
					...
					<span style="float: right; line-height: 2.5em;"> <?php
						...
						if ( !empty( $_GET['page'] ) && $_GET['page'] == "smart-manager" && !( !empty( $_GET['sm_old'] ) && ( 'woo' === $_GET['sm_old'] || 'wpsc' === $_GET['sm_old'] ) ) ) {
							...
							if ( SMBETAPRO === true ) {
								...
								$sm_old_link = '<a href="https://demo.storeapps.org/?demo=sm-woo&utm_source=in_app&utm_medium=sm_links&utm_campaign=lite_demo_link" target="_livedemo" title="'. __( 'Smart Manager Pro Demo', self::$text_domain ) .'"> ' . __( 'Pro Demo', self::$text_domain ) .'</a> | ';	
								' <a href="> ';
							}

							if( !empty( $sm_old ) ) {
								$sm_old_link .= '<a href="'. admin_url('admin.php?page='. $_GET['page'] .'&sm_old='. $sm_old) .'" title="'. __( 'Switch back to Smart Manager Old', self::$text_domain ) .'"> ' . __( 'Switch back to Old', self::$text_domain ) .'</a>';	
							}
						...
						printf ( __ ( '%1s%2s' , self::$text_domain), $sm_old_link, $before_plug_page );
						?>
					</span>
				</div>
			</div>
		...
		function smart_manager_support_ticket_content() {
			...
			StoreApps_Upgrade_3_2::support_ticket_content( 'sa_smart_manager_beta', self::$sku, $plugin_data, $license_key, self::$text_domain );
		}
